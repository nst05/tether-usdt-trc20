...
one 512-KiB внешняя SPI image.  ``small`` is a writable view of SPI 0x0000..0x1FFF.
...
CRC_SCHEMES = {
    ...
    "msp432": ("MSP4xx · CRC-16 CCITT 0x1021", crc16_msp432),
...
# MSP4xx сдвиг 0x19E0, у STM32 — 0x1B98 (там дублируется область
# 0x0000..0x0467). Адреса резервов вычисляются по сдвигу профиля.
...
class CE208State:
    ...
    def detect_crc_scheme(self) -> tuple[str, dict[str, int]]:
        """Подбирает профиль процессора: побеждает тот, по чьей схеме сходится
        больше записей. Проверяются текущие энергетические банки, статические
        записи и их резервные копии; адрес резерва зависит от профиля.

        Если не сходится ни одна схема (пустой или чужой образ), остаётся схема
        по умолчанию — поведение для родных дампов V8530 не меняется.
        """
        hits: dict[str, int] = {}
    ...
    def backup_address(self, primary: int, scheme: str | None = None) -> int:
        ...
        name = scheme or getattr(self, "crc_scheme", DEFAULT_CRC_SCHEME)
        return primary + MIRROR_OFFSETS.get(name, 0x19E0)
    ...
    def write_descriptor_body(self, descriptor: Descriptor, body: bytes | bytearray, write_backup: bool = True) -> bytes:
        ...
        if write_backup and descriptor.path == "small" and descriptor.backup_path == "small":
            address = self.backup_address(descriptor.primary)
            if address + descriptor.length <= SMALL_SIZE:
    ...
    def synchronize_direction_pair(
        self,
        active_bank: int,
        reactive_bank: int,
        tariffs: Iterable[int],
        ...
        """Synchronize one factory direction pair: 0/2 (A+/R+) or 1/3 (A-/R-)."""
        if (active_bank, reactive_bank) not in ((0, 2), (1, 3)):
            raise ValueError("Допустимы только штатные пары банков 0/2 и 1/3")
        values = list(tariffs)
        if len(values) != 8:
            raise ValueError("Для пары нужно ровно восемь тарифов")
        if any(value < 0 or value >= 10**12 for value in values):
            raise ValueError("Тарифная ячейка выходит за штатный диапазон")
        if not 0 <= reactive_ratio <= 1_000_000:
            raise ValueError("Коэффициент реактивной энергии вне диапазона")
        reactive_values = [round(value * reactive_ratio) for value in values]
        if any(value >= 10**12 for value in reactive_values):
            raise ValueError("Реактивная тарифная ячейка выходит за штатный диапазон")

        try:
            active, _ = self.read_current_energy(active_bank)
        except ValueError:
            active = EnergyBank.empty()
        active.set_tariffs(values)
        self.write_current_energy(active_bank, active)

        try:
            reactive, _ = self.read_current_energy(reactive_bank)
        except ValueError:
            reactive = EnergyBank.empty()
        reactive.set_tariffs(reactive_values)
        self.write_current_energy(reactive_bank, reactive)
        ...
        if include_archives:
            for archive_type, (_base, count, _name) in ENERGY_ARCHIVES.items():
                ...
                for slot in range(count):
                    try:
                        active_archived, _ = self.read_archive_energy(
                            archive_type, slot, active_bank
                        )
                    except ValueError:
                        active_archived = EnergyBank.empty()
                        active_archived.marker = archive_marker
                    active_archived.set_tariffs(values)
                    self.write_archive_energy(
                        archive_type, slot, active_bank, active_archived
                    )

                    try:
                        reactive_archived, _ = self.read_archive_energy(
                            archive_type, slot, reactive_bank
                        )
                    except ValueError:
                        reactive_archived = EnergyBank.empty()
                        reactive_archived.marker = archive_marker
                    reactive_archived.set_tariffs(reactive_values)
                    self.write_archive_energy(
                        archive_type, slot, reactive_bank, reactive_archived
                    )
                    written += 2
                per_type[archive_type] = written
                archive_records += written

        return {
            "active_bank": active_bank,
            "reactive_bank": reactive_bank,
            "tariffs": values,
            "reactive_tariffs": reactive_values,
            "reactive_ratio": reactive_ratio,
            "current_physical_records": 4,
            "archive_records": archive_records,
            "archive_records_by_type": per_type,
        }
