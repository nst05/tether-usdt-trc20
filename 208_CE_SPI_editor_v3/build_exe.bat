...
echo [1/3] Proverka PyInstaller...
%CE208_PYTHON% -m PyInstaller --version >nul 2>&1
if errorlevel 1 (
    echo PyInstaller ne naiden. Ustanovka...
    %CE208_PYTHON% -m pip install pyinstaller || goto :error
)
...
echo [3/3] Sborka CE208_SPI_Editor_v3.exe...
%CE208_PYTHON% -m PyInstaller --noconfirm ce208_editor.spec || goto :error

echo.
echo Gotovo: dist\CE208_SPI_Editor_v3.exe
pause
exit /b 0
