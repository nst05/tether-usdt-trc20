...
BOOT_FOOTER = (
    "три профиля контроля · запись 0x44 Б · 13 ячеек u40le · 4 банка · две группы тарифов"
    " | "
...
BOOT_PARAMETERS = [
    ("Внешняя SPI", "внешняя SPI · 0x80000"),
...
class Editor(tk.Tk):
    def __init__(self, show_splash: bool = True, crc_mode: str = "auto") -> None:
        ...
        self.source_kind = "spi"  # "spi" (512К внешняя SPI) или "24lc64" (8К внутр. EEPROM)
        ...
        self.status_var = tk.StringVar(value="Откройте дамп 24C64 (8 КиБ) или внешняя SPI (512 КиБ)")
    ...
    def _warning_text(self) -> str:
        if getattr(self, "source_kind", "spi") == "24lc64":
            return (
                ...
                "(small-path). Архивы и журналы событий лежат во внешней внешняя SPI (512 КиБ)."
            )
        return (
            "Загружена внешняя внешняя SPI (512 КиБ): архивы, резервные области и события. Логический small-path — "
    ...
    def _build_header(self) -> None:
        ...
        tk.Label(titles, text="Редактор энергонезависимой памяти · прошивка 10.14 · внешняя SPI / EEPROM 24C64",
    ...
    def _build_toolbar(self) -> None:
        ...
        processor_box = ttk.Combobox(toolbar, values=list(CRC_MODES), textvariable=self.crc_mode,
                                     width=14, state="readonly", font=(self.mono_font, 10))
    ...
    def check_processor_match(self, ask: bool = False) -> bool:
        ...
        При жёстком выборе (V8530 или MSP4xx) образ чужого прибора не читается:
    ...
    def processor_write_preflight(self) -> bool:
        """Block a write until the processor-specific record CRC is unambiguous.

        V8530 and MSP4xx use the same persistent layout, but a record written
        with the other processor's CRC is rejected by firmware.  A manual
        processor choice is allowed for an empty/damaged image; a detected
        conflict is never allowed.
        """
        if not self.at25_loaded:
            messagebox.showerror(APP_TITLE, "Сначала откройте исходный дамп прибора.")
            return False

        model = self.state_model
        hits = dict(getattr(model, "crc_scheme_hits", {}) or {})
        forced = getattr(model, "crc_forced", None)
        detected = getattr(model, "crc_detected", None)

        if forced:
            other_hits = max(
                (count for name, count in hits.items() if name != forced),
                default=0,
            )
            if hits.get(forced, 0) == 0 and other_hits > 0:
                self.check_processor_match()
                messagebox.showerror(
                    APP_TITLE,
                    "Запись заблокирована: выбранный процессор не соответствует дампу. "
                    "Выберите подходящий процессор или режим «Авто».",
                )
                return False
            model.crc_scheme = forced
            set_crc_scheme(forced)
            return True

        best_hits = max(hits.values(), default=0)
        winners = [name for name, count in hits.items() if count == best_hits and count > 0]
        if best_hits == 0 or len(winners) != 1 or detected not in winners:
            messagebox.showerror(
                APP_TITLE,
                "Запись заблокирована: процессор по дампу не определяется однозначно. "
                    "Выберите вручную V8530, MSP4xx или STM32; программа применит соответствующую CRC.",
            )
            return False

        model.crc_scheme = detected
        set_crc_scheme(detected)
        return True
    ...
    def _update_side_panel(self) -> None:
        ...
        self.side_vars["Память"].set("24C64 внутр." if self.source_kind == "24lc64" else "внешняя SPI внешн.")
    ...
    def _energy_controls(self, parent: ttk.Frame, archive: bool) -> tuple[list[tk.StringVar], tk.StringVar, tk.StringVar, tk.StringVar, tk.StringVar]:
        ...
        ttk.Label(
            parent,
            text="Тарифы архива" if archive else "Группа 1 · 1.8.x (A+) / 3.8.x (R+)",
    ...
    def _ensure_group2_variables(self) -> None:
        if hasattr(self, "energy_group2_enabled"):
            return
        self.energy_group2_enabled = tk.BooleanVar(value=False)
        self.energy_group2_values = [tk.StringVar(value="0.00000") for _ in range(8)]
        self.energy_group2_sum = tk.StringVar(value="∑ Группа 2: —")
        self.energy_group2_entries: list[ttk.Entry] = []
        for variable in self.energy_group2_values:
            variable.trace_add("write", self.preview_group2_sum)

    def _add_group2_editor(self, parent: ttk.Frame) -> None:
        self._ensure_group2_variables()
        switch = ttk.Frame(parent)
        switch.pack(fill="x", pady=(8, 4))
        ttk.Checkbutton(
            switch,
            text="Редактировать вторую группу тарифов (2.8.x / 4.8.x)",
            variable=self.energy_group2_enabled,
            command=self._toggle_group2,
        ).pack(side="left")
        ttk.Label(
            parent,
            text="Группа 2 · 2.8.x (A−) / 4.8.x (R−)",
            style='Section.TLabel',
        ).pack(anchor="w", pady=(4, 4))
        grid = ttk.Frame(parent)
        grid.pack(fill="x", pady=(0, 6))
        for index, variable in enumerate(self.energy_group2_values):
            row, column = divmod(index, 4)
            base = column * 2
            ttk.Label(grid, text=f"T{index + 1}", font=('TkDefaultFont', 10, 'bold')).grid(
                row=row, column=base, sticky="e", padx=8, pady=4
            )
            entry = ttk.Entry(grid, textvariable=variable, width=18)
            entry.grid(row=row, column=base + 1, sticky="w", padx=8, pady=4)
            self.energy_group2_entries.append(entry)
        ttk.Label(parent, textvariable=self.energy_group2_sum, style="Sum.TLabel").pack(
            anchor="w", pady=(2, 8)
        )
        self._toggle_group2()

    def _toggle_group2(self) -> None:
        state = "normal" if self.energy_group2_enabled.get() else "disabled"
        for entry in getattr(self, "energy_group2_entries", []):
            entry.configure(state=state)
        self.preview_group2_sum()
    ...
    def _build_energy_tab(self) -> None:
        ...
        self._add_group2_editor(self.energy_tab)
        ...
        ttk.Checkbutton(
            react,
            text="Авто-реактивная для включённых групп: 3.8=1.8×K, 4.8=2.8×K",
        ...
        for variable in (self.energy_divisor, self.energy_decimals):
            variable.trace_add("write", self.preview_group2_sum)
        ...
        ttk.Button(big, text="Прочитать обе группы", style="Ghost.TButton",
                   command=self.load_tariff_groups).pack(side="left")
    ...
    def _build_direct_tab(self) -> None:
        ...
        ttk.Label(self.direct_tab, text="Группа 1 · 1.8.x / 3.8.x", style='Section.TLabel').pack(anchor="w", pady=(4, 4))
        ...
        self._add_group2_editor(self.direct_tab)

        ttk.Checkbutton(self.direct_tab, text="Авто-реактивная для обеих включённых групп × K",
    ...
    def open_at25(self) -> None:
        path = filedialog.askopenfilename(
            title="Дамп памяти: 24C64 (8 КиБ) или внешняя SPI (512 КиБ)",
        ...
        if len(raw) == SMALL_SIZE:
            ...
            self.at25_var.set(f"внешняя SPI: {Path(path).name}  (512 КиБ, архивы)")
            self.status_var.set("Внешняя внешняя SPI загружена")
        else:
            messagebox.showerror(
                APP_TITLE,
                f"Нужен дамп 24C64 ({SMALL_SIZE} б) или внешняя SPI ({AT25_SIZE} б); получено {len(raw)} б",
            )
    ...
    def save_spi(self) -> None:
        ...
        if self.source_kind == "24lc64":
            out_path = filedialog.asksaveasfilename(
                title="Сохранить 24C64 (8 КиБ)", defaultextension=".bin", initialfile="CE208_24C64_edited.bin"
            )
            if not out_path:
                return
            ...
            self._set_busy(False, f"24C64 (8 КиБ) сохранён; отчёт: {audit_path.name}")
            self.activity.pulse(1.0)
            return
        at25_path = filedialog.asksaveasfilename(title="Сохранить внешняя SPI", defaultextension=".bin", initialfile="CE208_внешняя SPI_edited.bin")
        if not at25_path:
            return
    ...
    def apply_clock(self) -> None:
        try:
            if not self.processor_write_preflight():
                return
    ...
    def preview_group2_sum(self, *_args) -> None:
        if not hasattr(self, "energy_group2_sum"):
            return
        if not self.energy_group2_enabled.get():
            self.energy_group2_sum.set("∑ Группа 2 отключена — банки 1/3 сохраняются без изменений")
            return
        self._preview_sum(
            self.energy_group2_values,
            self.energy_tariff_count,
            self.energy_divisor,
            self.energy_decimals,
            self.energy_group2_sum,
        )
    ...
    def load_tariff_groups(self) -> None:
        """Load A+ (bank 0) and A- (bank 1) into their independent tariff fields."""
        loaded: list[str] = []
        try:
            first, first_result = self.state_model.read_current_energy(0)
            self._load_energy_vars(
                first,
                self.energy_values,
                self.energy_divisor.get(),
                self.energy_decimals.get(),
                self.energy_sum,
                self.energy_tariff_count,
            )
            loaded.append(f"группа 1 @0x{first_result.address:04X}")
        except ValueError:
            first = EnergyBank.empty()
            self._load_energy_vars(
                first,
                self.energy_values,
                self.energy_divisor.get(),
            ...
            loaded.append("группа 1 пустая")

        try:
            second, second_result = self.state_model.read_current_energy(1)
            self._load_energy_vars(
                second,
                self.energy_group2_values,
                self.energy_divisor.get(),
            ...
            loaded.append(f"группа 2 @0x{second_result.address:04X}")
        except ValueError:
            second = EnergyBank.empty()
            self._load_energy_vars(
                second,
                self.energy_group2_values,
                self.energy_divisor.get(),
            ...
            loaded.append("группа 2 пустая")
        self.preview_group2_sum()
        self.energy_bank.set("0")
        self.energy_info.set(
            "; ".join(loaded)
            + ". Пары: банки 0/2 = 1.8/3.8; банки 1/3 = 2.8/4.8."
        )
    ...
    def _energy_filename(self, ext: str = ".bin") -> str:
        """Имя файла из независимых значений групп 1/2 и их сумм."""
        try:
            decimals = int(self.energy_decimals.get(), 0)
        except Exception:
            decimals = 2
        parts: list[str] = []
        groups = [("G1", self.energy_values)]
        if self.energy_group2_enabled.get():
            groups.append(("G2", self.energy_group2_values))
        for prefix, variables in groups:
            ...
            for i, var in enumerate(variables):
                ...
                try:
                    ...
                    value = 0.0
                if value > 0:
                    group_parts.append(f"{prefix}T{i + 1}-{value:.{decimals}f}")
                    total += value
            parts.extend(group_parts or [f"{prefix}-empty"])
            parts.append(f"{prefix}SUM-{total:.{decimals}f}")
        if not parts:
            parts = ["empty"]
        name = "_".join(parts)
    ...
    def apply_and_write(self, force_direct: bool = False, progressbar=None) -> None:
        ...
        try:
            ...
            # Два режима намеренно различаются:
            # - обычное редактирование полного SPI обновляет и энергетические архивы;
            # - прямая запись через CH341 относится к 8-КБ EEPROM и архивы SPI/события
            #   затрагивать не должна, даже если перед этим в редактор был открыт полный SPI.
            update_spi_archives = self.source_kind == "spi" and not force_direct

            # 1) Часы = текущее время
            self.state_model.write_clock(ClockValue.from_datetime(now))
            # 2) Независимые активные группы: 0=1.8.x, 1=2.8.x.
            group1 = [encode_energy(v.get(), divisor, decimals) for v in self.energy_values[:8]]
            pairs = [("G1", 0, 2, group1)]
            if self.energy_group2_enabled.get():
                group2 = [
                    encode_energy(v.get(), divisor, decimals)
                    for v in self.energy_group2_values[:8]
                ]
                pairs.append(("G2", 1, 3, group2))
            k = float(self.reactive_k.get()) if self.reactive_auto.get() else 0.0
            marker = int(self.energy_sync_marker.get(), 0)
            pair_notes: list[str] = []
            archive_records = 0
            for name, active_bank, reactive_bank, values in pairs:
                if self.reactive_auto.get():
                    stats = self.state_model.synchronize_direction_pair(
                        active_bank,
                        reactive_bank,
                        values,
                        k,
                        marker,
                        include_archives=update_spi_archives,
                    )
                    archive_records += stats["archive_records"]
                    pair_notes.append(f"{name}:{active_bank}/{reactive_bank}")
                elif update_spi_archives:
                    stats = self.state_model.synchronize_energy_everywhere(
                        [active_bank], values, marker
                    )
                    archive_records += stats["archive_records"]
                    pair_notes.append(f"{name}:{active_bank}, реактивная сохранена")
                else:
                    try:
                        active, _ = self.state_model.read_current_energy(active_bank)
                    except ValueError:
                        active = EnergyBank.empty()
                    active.set_tariffs(values)
                    self.state_model.write_current_energy(active_bank, active)
                    pair_notes.append(f"{name}:{active_bank}, реактивная сохранена")
            self.state_model.write_active_tariff(int(self.active_tariff.get(), 0))

            # 3) Счётчики времени = сейчас
            try:
                for blk in (0, 1):
                    self.state_model.write_time_counters(blk, TimeCounterBlock.initialized(now))
            except Exception:
                pass
            # 4) Для полного SPI архивы уже синхронизированы выше; добавляем одно событие.
            if update_spi_archives:
                try:
            ...
            self._stage(
                bar,
                "Записаны " + ", ".join(pair_notes)
                + (f"; архивных записей {archive_records}" if update_spi_archives else "; только текущие записи"),
                25,
            )
            self.update_idletasks()
            ...
            steps = [
                "часы",
                "пары " + ",".join(pair_notes),
                "архивы " + str(archive_records) if update_spi_archives else "без архивов SPI",
                "счётчики",
            ]
            saved = None
            ...
            self._set_busy(False)
            self.load_clock(); self.load_active_tariff(); self.load_tariff_groups()
    ...
    def apply_consistent_write(self) -> None:
        ...
        try:
            ...
            archive_note = (
                ...
                if is_spi else
                "(архивы/события — в внешняя SPI; тут 24C64: часы+банки+тариф+счётчики)"
            )
    ...
    def refresh_record_tree(self) -> None:
        ...
        valid_count = 0
        available_count = 0
        # Открыта только внутренняя память (8 КиБ) — записи внешней SPI в файл
        ...
        for index, descriptor in enumerate(FIXED_DESCRIPTORS):
            if only_small and descriptor.path != "small":
            ...
            self.record_tree.insert("", "end", iid=str(index), tags=(tag, stripe),
                                    values=(descriptor.name, descriptor.path, f"0x{descriptor.primary:05X}",
                                            backup, descriptor.length, status))
...
def main() -> None:
    ...
    for value in sys.argv[1:]:
        if value.startswith("--crc"):
            ...
            crc_mode = {"v8530": "ce208", "msp4xx": "msp432"}.get(crc_mode, crc_mode)
    if crc_mode == "msp430":  # совместимый псевдоним профиля MSP4xx
        crc_mode = "msp432"
    app = Editor(crc_mode=crc_mode if crc_mode in ("auto", "ce208", "msp432", "stm32") else "auto")
    if arguments:
        ...
        if candidate.exists():
            ...
            if len(raw) == SMALL_SIZE:
                ...
                app.at25_path = candidate
                app.at25_loaded = True
                app.at25_var.set(f"внешняя SPI: {candidate.name}  (512 КиБ, архивы)")
            else:
                messagebox.showerror(APP_TITLE, f"Нужен дамп 24C64 ({SMALL_SIZE} б) или внешняя SPI ({AT25_SIZE} б)")
            app.refresh_all()
    app.mainloop()
