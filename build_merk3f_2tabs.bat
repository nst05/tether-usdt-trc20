...
echo ================================================================
echo   Sborka MERK-3F (bez 231-AT-01)
echo ================================================================
echo.
...
set "MISSING="
for %%f in (srt_mod_spi.py spi_memory.py splash_screen.py make_icon.py pyi_rth_ch341.py merk3f_2tabs.py merk3f_2tabs.spec) do (
...
if exist "dist\MERK-3F_2tabs.exe" del /q "dist\MERK-3F_2tabs.exe"

%PY% -m PyInstaller --noconfirm --clean merk3f_2tabs.spec
if errorlevel 1 (
    echo.
...
if not exist "dist\MERK-3F_2tabs.exe" (
    echo [OSHIBKA] exe ne sozdan, hotya PyInstaller ne soobshchil ob oshibke.
    goto :fail
)

set "SIZE=0"
for %%f in ("dist\MERK-3F_2tabs.exe") do set /a SIZE=%%~zf/1048576
...
echo ================================================================
echo.
echo   Fayl:    %CD%\dist\MERK-3F_2tabs.exe
echo   Razmer:  !SIZE! MB
echo   Vkladki: ART, AR
echo.
if not exist "!DLL!" (
