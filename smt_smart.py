...
# --------------------------------------------------------------------------- #
RECORD_SIZE = DEFAULT_PROFILE.state_record_size
DATA_OFF = DEFAULT_PROFILE.smart_data_offset                     # блок данных внутри супер-записи
...
def find_records(buf: bytes) -> list[int]:
    ...
    while True:
        ...
        if DEFAULT_PROFILE.smart_min_unix_ts <= ts <= DEFAULT_PROFILE.smart_max_unix_ts:      # 2020..2035
            bases.append(base)
    return bases
