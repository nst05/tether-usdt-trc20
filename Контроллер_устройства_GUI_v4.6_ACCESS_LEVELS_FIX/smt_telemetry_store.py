...
def _auth_ok(report: Dict[str, Any]) -> Optional[int]:
...
def _last_record(con: sqlite3.Connection, id64: str) -> Optional[sqlite3.Row]:
    if not id64:
        return None
    return con.execute(
...
def ingest(path: str, raw: bytes, report: Dict[str, Any], *,
           source_ip: str = "", source_port: Optional[int] = None) -> Tuple[int, bool, int]:
    ...
    with _connect(path) as con:
        ...
        previous = _last_record(con, id64)
...
def recent_records(path: str, limit: int = 500, *, anomalies_only: bool = False,
    ...
    if id64:
        where.append("p.id64=?"); args.append(id64)
