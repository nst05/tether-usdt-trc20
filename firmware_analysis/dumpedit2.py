...
# Все 8 checksum-защищённых записей (копируются/проверяются функциями 0x81FC
# из 0x8150 и 0x14E0). 8-словные записи — параметры калибровочной кривы канала
# (обрабатываются интерполяцией в 0x6C8C), поэтому слова подписаны индексом.
RECORDS = [
    (0x1000, 8, 'Калибровка канала 1 (кривая)', {}),
    (0x1014, 8, 'Калибровка канала 2 (кривая)', {}),
    (0x1028, 8, 'Калибровка канала 3 (кривая)', {}),
    ...
    (0x1080, 4, 'Конфигурация A (0x1080)', {}),
    (0x1090, 4, 'Конфигурация B (0x1090)',
     {1: 'выбор набора калибровки (мл.4 бита)'}),
]
