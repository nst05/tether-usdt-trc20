...
R0, R1 = DEFAULT_PROFILE.event_log_start, DEFAULT_PROFILE.event_log_end
MIN_EVENT_TS = DEFAULT_PROFILE.event_min_unix_ts
MAX_EVENT_TS = DEFAULT_PROFILE.event_max_unix_ts
...
def parse(data):
    ...
    o = R0
    limit = min(R1, len(data))
    while o < limit - 16:
        t, cnt = u32(o), u32(o + 8)
        if MIN_EVENT_TS <= t <= MAX_EVENT_TS and cnt < 100000:
            code = u16(o + 0x0C)
            e = data.find(b"\0", o + 0x0E, min(limit, o + 0x0E + 48))
            if e < 0:
                e = min(limit, o + 0x0E + 48)
            raw = data[o + 0x0E:e]
            txt = raw.decode("latin1") if (0 < len(raw) < 48 and all(31 <= b < 127 for b in raw)) else ""
            # При завороте/повторе счётчика сохраняется самое новое событие,
            # интерфейс parse() остаётся совместимым: ключом по-прежнему служит cnt.
            previous = seen.get(cnt)
...
def main():
    ...
    ts = [v[0] for v in seen.values() if MIN_EVENT_TS <= v[0] <= MAX_EVENT_TS]
    ...
    items = sorted(seen.items(), key=lambda item: (item[1][0], item[0]))
    ...
    if any(v == "123456" for _, v in secrets) or \
       any(item[2] == "123456" for item in seen.values()):
